...
This plugin exposes three Modus MCP servers:

- **`modus-assistant`** — full-environment Modus (homepage agent): `chat_with_modus`, `get_modus_context`, `get_context_item`. Use when you want Modus itself across the whole org.
- **`modus-context`** — discover published **skills** and **agents**, chat with them, and pull raw org context. This is what you use for most org questions that target a specific expert.
...
## Assistant workflow (full-environment Modus)

On **`modus-assistant`**:

1. **`chat_with_modus(message, model)`** — same experience as the Modus home page. Pass `threadId` from the response to continue.
2. **`get_modus_context(message)`** — compose full-environment context without generating an answer.
3. **`get_context_item(sessionId, path)`** — read entries from `get_modus_context` (modus sessions only).
