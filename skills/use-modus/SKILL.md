...
- **`modus-context`** — discover published **skills** and **workflows**, chat with them, and pull raw org context. This is what you use for most org questions.
- **`modus-manage`** — create / update / deploy / delete skills, workflows, and context items. Administrative; use only on explicit admin requests.
...
## Context workflow (skills & workflows)

1. **Find the right expert.** Call `list_skills` (optionally with a `query`) to see the org's published skills, or `list_workflows` for workflows.
2. **Ask it.** Call `chat_with_skill(skillId, message)` — returns JSON with `content` and `threadId`; pass `threadId` on follow-ups. For workflows, use `chat_with_workflow(workflowId, message)`.
...
Use `get_skill(skillId)` / `get_workflow(workflowId)` when you need one resource's details.

## Organization (admin only)

The **`modus-manage`** tools (`skills_*`, `workflows_*`, `context_*`) **change** the org's Modus configuration. Use them only when the user explicitly asks to create, edit, deploy, or delete something. Destructive operations require an explicit confirmation flag.
